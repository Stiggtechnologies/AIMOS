...
export default function WorkOrdersView() {
  ...
  async function fetchWorkOrders() {
    try {
      let query = supabase
        .from('work_orders')
        .select('*, assets(name, asset_id), auth.users!assigned_to(full_name)')
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
          <div className="flex items-center gap-2 text-blue-400 text-sm"><AlertTriangle className="w-4 h-4" /> Scheduled</div>
          ...
          <tbody className="divide-y divide-slate-700">
            {workOrders.map((wo: any) => (
              <tr key={wo.id} className="hover:bg-slate-700/50">
                ...
                <td className="px-4 py-3 text-slate-300">{wo.assigned_to?.full_name || 'Unassigned'}</td>
                <td className="px-4 py-3 text-white">${wo.total_cost?.toFixed(2) || '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {workOrders.length === 0 && <p className="p-8 text-center text-slate-400">No work orders found</p>}
      </div>
    </div>
  );
}
